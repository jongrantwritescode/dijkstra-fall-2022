// Convert kebab case words into camel casing. The first word within the output should be capitalized only if it is the first letter of a word (except the first one) and dashes should be removed.

// change-this-word gets converted into changeThisWord
// day-month-year gets converted into dayMonthYear
// ALL-CAPS gets converted into allCaps

function kebabToCamel(kebabString) {
    let camelStrimg;
   
    //loop over kebabstring 
    // - if the char dash, don't use it AND the following char would be capitalized
    // - otherwise, copy lowercased to camelString

    // test-string => testString
    return camelStrimg;
}


